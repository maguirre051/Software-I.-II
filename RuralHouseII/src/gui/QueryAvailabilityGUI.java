...
private static final long serialVersionUID = 1L;
  ...
  private void jbInit() throws Exception
  {
	...
	for (int i=0; i<rhs.size(); i++){
		b.addElement(rhs.get(i).getCity());
	}	
	comboBox = new JComboBox<String>(b);
...
//    });
 ...
 {		
	 ...
	 ApplicationFacadeInterface facade =  MainWindow.getBusinessLogic();
 		...
 		Vector<RuralHouse> etxe = new Vector<RuralHouse>();
		try {
		...
		for (int i=0; i<etxe.size(); i++){
			if (comboBox.getSelectedItem().equals((etxe.get(i)).getCity())){
				r = etxe.get(i);
			}
		}
    	...
    	try {
    		...
    		Vector<Offer> v=r.getOffers(firstDay, lastDay);
